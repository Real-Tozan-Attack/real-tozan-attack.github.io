function geojson_style(prop) {
  ...
  xhr1.send(null);
  var NHM_Mtlist_Data = JSON.parse(xhr1.responseText);
  var NHM_Done_marker = L.geoJson(NHM_Mtlist_Data, {
    filter: function RTATypeFilter(feature){
  	  if(feature.properties._Attacked === "-") return true
    },
  pointToLayer: function (feature, latlng) {
    ...
    onEachFeature: function (feature, layer) {
      layer.bindTooltip(tooltip_properties(feature.properties.番号) + "<br>" + tooltip_properties(feature.properties._走者画像),{permanent: true, direction: 'top', className: 'RTA_ReachedMtlist-tooltip'});
  ...
  var NHM_Done_Layer = markers1.addLayer(NHM_Done_marker);

  //日本百名山(未攻略)
  var NHM_Ny_marker = L.geoJson(NHM_Mtlist_Data, {
    filter: function RTATypeFilter(feature){
  	  if(feature.properties._Attacked === "未攻略") return true
    },
    pointToLayer: function (feature, latlng) {
      var s = geojson_style(feature.properties);
      if(feature.properties['_markerType']=='Icon'){
  ...
  var NHM_Ny_Layer = markers2.addLayer(NHM_Ny_marker);


  var map = L.map('mapdiv', {
   center: [36.104700,140.087013], layers: [gsi, NHM_Done_Layer,NHM_Ny_Layer]
  });

  L.control.scale({imperial: false}).addTo(map);

  var baseLayers = {};
  var overlays = {"日本百名山(攻略済み)": NHM_Done_Layer, "日本百名山(未攻略)": NHM_Ny_Layer};
  ...
  var searchLayers = L.layerGroup([NHM_Done_Layer]);
  ...
  map.fitBounds(NHM_Done_Layer.getBounds());
  ...
  var lastupdate = NHM_Mtlist_Data.lastupdate
